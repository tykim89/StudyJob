...
function ou(obj){
  ...
  function popupOpen(year,month,day){
	  var popUrl = "${pageContext.request.contextPath }/scaduleContent.do?no=${studyTitle.study_no}&date="+year+"/"+month+"/"+day;	//!!!!!회원정보 페이지랑 연결하기!!!!!
			...
			<div style='padding: 4px 8px;'>
				<div style='position: relative;'>
					<table align="center" width="430" height="50" cellpadding="2" cellspacing="1">
						<tr>
						<td align="center">
							<a href="${pageContext.request.contextPath }/controller/calendarview.do?no=${studyTitle.study_no}&year=<%=preYear %>&month=<%=preMonth %>"><font size =5>◀</font></a>
							<b>&nbsp;<font size =5><%=year %>년</font>&nbsp;&nbsp;<font size =5><%=month %>월</font></b>
							<a href="${pageContext.request.contextPath }/controller/calendarview.do?no=${studyTitle.study_no}&year=<%=nextYear %>&month=<%=nextMonth %>"><font size =5>▶</font></a>
							</td>
						</tr>
					</table>
				</div>
...
%>
	...
	<c:when test="${flag== true }">
	 <td style='line-height:120%; cursor:pointer' onmouseover='overM(this);' onmouseout='ou(this);' onclick="popupOpen(${year},${month},${day});" >
	</c:when>			
	<c:otherwise>
	 <td style='line-height:120%;'>
				...
				<div class=nor ><span id='indate'><font color= <%=fontColor %>><%=i%></font></span>
						...
						<%-- <span class='tdBox'><%=year+"/"+month+"/"+i %></span> --%>
